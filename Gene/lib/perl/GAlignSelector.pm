...
package DoTS::Gene::GAlignSelector;
...
#
sub new {
    my($class, $taxon_id, $chrom_id, $opts) = @_;

    croak(__PACKAGE__ . "::new: taxon_id/chrom_id required")
            unless $taxon_id =~ /^\d+$/ && $chrom_id =~ /^\d+$/;

    my $self = {
        taxon_id => $taxon_id,     # what species is this for?
	chrom_id => $chrom_id,     # on which chromosome?
        start => $args->{start},
        end => $args->{end},
